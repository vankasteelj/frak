  ...
  test: () => {
    ...
    Search.testOnline().then(results => {
      ...
      for (const i in results.shows) {
        const item = `<tr><td>${results.shows[i].tested}</td><td><div ${results.shows[i].ms ? "title=" + results.shows[i].ms + "ms" : ""} class="fa ${check(results.shows[i].working)}"></div></td><td><div ${results.movies[i].ms ? "title=" + results.movies[i].ms + "ms" : ""} class="fa ${check(results.movies[i].working)}"></div></td></tr>`
